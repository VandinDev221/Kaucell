...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        $rows = $pdo->query("SELECT id, nome, categoria, preco, imagem_url, imagem_arquivo FROM produtos ORDER BY id DESC")->fetchAll();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO produtos (nome, categoria, preco, imagem_url, imagem_arquivo)
            VALUES (:nome, :categoria, :preco, :imagem_url, :imagem_arquivo)
            RETURNING id, nome, categoria, preco, imagem_url, imagem_arquivo
        ");
        $stmt->execute([
            ':nome' => $nome,
            ':categoria' => $categoria,
